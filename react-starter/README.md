...
Working progress... NodeJS is not install on this machine. Current process I had to create a Node container then go exec into the container, remove all files and then `npm create-react-app` to obtain all required files and starter set up. Next brought down the container, then `docker-compose run nodejs npm install` and finally `docker-compose up` to have a running environment.
